...
type
  ArrowArrayInternal[T] = object 
    offsets: WrappedBufferPtr[uint32]
    data: WrappedBufferPtr[T]
    nullBitmap: WrappedBufferPtr[NullBitmapBase]
  ...
  WrappedBuffer[T] = object 
    raw: ptr UncheckedArray[T]
    buf: GArrowBufferPtr
    bytes: int64
    length: int64

  WrappedBufferPtr[T] = ref WrappedBuffer[T]
...
proc reserveBytes[T](b: WrappedBufferPtr[T], bytes: int64) =  
  if b.raw == nil:
...
proc reserveElements[T](b: WrappedBufferPtr[T], elems: int64) =
  if b.raw == nil:
...
proc addElement[T](b: WrappedBufferPtr[T], elem: T) =    
  b.reserveElements(b.length + 1)
  b.raw[b.length] = elem
  b.length += 1

proc addBytes[T](b: WrappedBufferPtr[T], data: pointer, size: Natural) =
  b.reserveBytes(b.length + size)
  let dest = cast[ptr UncheckedArray[char]](b.raw)
  copyMem(addr dest[b.length], data, size)
  b.length += size

proc setGlibBuffer[T](b: WrappedBufferPtr[T]) =
...
proc `=destroy`*[T](x: var WrappedBuffer[T]) =
  if x.raw != nil:
    dealloc(x.raw)
...
proc copyToBuffer[T](arr: openArray[T]): WrappedBufferPtr[T] =
...
proc emptyBuffer[T](): WrappedBufferPtr[T] =  
  WrappedBufferPtr[T](
    raw: nil,
    buf: nil,
    bytes: 0,
...
proc newArrowArray[T](offsets: WrappedBufferPtr[uint32], data: WrappedBufferPtr[T], 
                      nullBitmap: WrappedBufferPtr[NullBitmapBase], 
